...
const Cart = () => {
  ...
  useEffect(()=>{
   const totals = cartProduct.reduce((sum,product)=>{
    sum.regular = sum.regular + product?.regularPrice * product?.quantity;
    sum.discounted = sum.discounted + product?.discountedPrice * product?.quantity;
    ...
    <Container>
      {cartProduct?.length > 0 ? (
        ...
        <p className="text-lg max-w-[600px] text-center text-gray-600 tracking-wide leading-6 mb-6">
          Your cart is empty. Go make some shopping...
        </p>
        <Link
          to="/product"
