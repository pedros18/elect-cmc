...
import { FiStar, FiUser, FiShoppingBag } from 'react-icons/fi';
  ...
  return (
    <div className='w-full bg-whiteText md:sticky md:top-0 z-50'>
      <div className='max-w-screen-xl mx-auto h-20 flex items-center justify-between px-4 lg:px-0'>
        <Link to="/" className='font-bold text-redText text-xl/10'>
          ELECT-CMC
        </Link>

        <div className='hidden md:inline-flex flex-1 mx-5 max-w-3xl relative'>
          <input
            type="text"
        ...
        <div>
          {searchText && (
            <div className='absolute left-0 top-20 w-full mx-auto max-h-[500px] px-10 py-5 bg-white z-20 overflow-y-scroll text-black shadow-lg shadow-skyText scrollbar-hide scrollbare-hide'>
              {filteredProducts.length > 0 ? (
                <div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-5'>
                  {filteredProducts.map((item: ProductProps) => (
                    <ProductCard product={item} key={item._id} setSearchText={setSearchText} />
                  ))}
                </div>
              ) : (
                <div className='py-10 bg-gray-50 w-full flex items-center justify-center border border-gray-600 rounded-md'>
                  <p className='text-xl font-normal '>
                    Nothing matches your search keywords{" "}
                    <span className='underline underline-offset-[1px] text-red-500 font-semibold'>
                      {`(${searchText})`}
                    </span>
                    . Please try again.
                  </p>
